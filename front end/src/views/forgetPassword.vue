			...
			<form method="POST">
				<div class="login-form">
					...
					<div class="form-item" ref="passwordResetRef" :model="passwordReset">
						...
						<input class="email" type="text" placeholder="Email" v-model="passwordReset.email"
							oninput="value= value.replace(/[^a-zA-Z0-9.]/g,'')">@student.xjtlu.edu.cn
...
<script>
	export default {
		...
		methods: {
			async submit() {
				if (this.passwordReset.username == '') {
				...
				if (!pattern1.test(this.passwordReset.email)) {
				...
				var data = {
					...
					email: this.passwordReset.email + "@student.xjtlu.edu.cn",
